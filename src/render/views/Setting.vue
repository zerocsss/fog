        ...
        <div class="setting-view-content">
          ...
          >
            ...
            >
              ...
              <fog-button size="mini">{{ t('setting.general.select_manually_button_text') }}</fog-button>
            </fog-form-item>
            <!-- IDE TODO -->
            <fog-form-item
            ...
            >
              <fog-radio-group
                ...
                disabled
              >
                <fog-radio value="Visio Studio Code">Visio Studio Code</fog-radio>
            ...
            <fog-divider></fog-divider>
            <!-- 自动fetch -->
            <fog-form-item field="autoFetch" :label="t('setting.general.auto_fetch_text')">
              <fog-radio-group
                type="button"
                size="mini"
                v-model="autoFetch"
              ...
              >
                ...
                <fog-radio
                  :value="AutoFetch.EvertHour"
                >{{ t('setting.general.auto_fetch.oneHour') }}</fog-radio>
              </fog-radio-group>
            </fog-form-item>
            <fog-divider></fog-divider>
            <!-- commit数量 -->
            <fog-form-item field="commitCount" :label="t('setting.general.commit_counts_text')">
              <fog-radio-group
                type="button"
                size="mini"
        ...
        <div class="setting-view-content">
          ...
          >
            ...
            >
              ...
              <template #extra>
                <div>{{ $t('welcome.content.setting.git.git_minimum_tip') }}</div>
              </template>
            </fog-form-item>
            <fog-divider></fog-divider>
            <!-- Git 全局username -->
            <fog-form-item field="gitUsername" :label="$t('setting.git.global_user_name_text')">
              <fog-input
                size="mini"
                v-model="globalName"
            ...
            <!-- Git 全局email -->
            <fog-form-item field="gitEmail" :label="$t('setting.git.global_email')">
              <fog-input
                size="mini"
                v-model="globalEmail"
            ...
            <fog-form-item
              field="gitGPGBinary"
              :label="$t('welcome.content.setting.git.git_gpg_binary_label_text')"
            >
              <!-- TODO 搜索GPG -->
              <fog-select
                :placeholder="$t('welcome.content.setting.git.git_gpg_binary_placeholder_text')"
                size="mini"
              ...
              <template #extra>
                <div>{{ $t('welcome.content.setting.git.gpg_minimum_tip') }}</div>
              </template>
            </fog-form-item>
            <!-- 全局GPG Key -->
            <fog-form-item
            ...
            >
              <!-- TODO GPG相关功能-->
              <fog-select
                :placeholder="$t('welcome.content.setting.git.git_gpg_key_placeholder_text')"
                size="mini"
                ...
                <template #footer>
                  <div style="padding: 6px 0; text-align: center;">
                    <fog-button
                      size="mini"
                      style="width: 90%;"
                    >{{ $t('welcome.content.setting.git.create_gpg_key_button_text') }}</fog-button>
                  </div>
                </template>
              </fog-select>
            </fog-form-item>
            ...
            <fog-typography-title type="secondary" :heading="6">
              ...
              <fog-button-group>
                <fog-button
                  @click="feedback"
                  size="small"
                >{{ $t("setting.about.feedback_btn_text") }}</fog-button>
...
import { getGlobalGitConfig, searchExistGitBinary, setGlobalGitEmail, setGlobalGitName } from "../utils/git";
...
const terminalApplicationChanged = (e: string) => store.commit("setTerminalApplication", e)
const IDEApplicationChanged = (e: string) => store.commit("setIDEApplication", e)
